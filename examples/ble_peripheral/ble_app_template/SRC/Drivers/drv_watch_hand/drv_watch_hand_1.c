...
{                                                   \
    ...
    {                                               \
        ...
        CONFIG_IO_WATCH_HAND_2_RIGHT | NRFX_PWM_PIN_INVERTED,         \
...
static nrfx_pwm_t                pwm1 = NRFX_PWM_INSTANCE(1); // 反推
...
static uint32_t pwm_init(void)
{
    ...
    {
        err_code = nrfx_pwm_init(&pwm1, &backward_config, pwm_handler);
    }
...
static void pwm_start(void)
{
    if (m_pin_active)
    {
        ...
        {
            nrfx_pwm_complex_playback(&pwm1, &backward_seq00, &backward_seq10, 2, 
    ...
    else
    {
        ...
        {
            nrfx_pwm_complex_playback(&pwm1, &backward_seq01, &backward_seq11, 2,
                                         NRF_DRV_PWM_FLAG_PERIOD_END | NRFX_PWM_FLAG_STOP | NRF_DRV_PWM_FLAG_STARTED_SEQ1);
        }
    }
