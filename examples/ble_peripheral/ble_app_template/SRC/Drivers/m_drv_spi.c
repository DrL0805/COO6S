...
// 返回值  :	无
//************************************************************************************************************
void  m_spi_gpio_uninit(spi_select_t sel)
{
	switch (sel)
	{
...
// 返回值  :	无
//************************************************************************************************************
void  m_spi_gpio_init(spi_select_t sel)
{
	switch (sel)
	{
		case SPI_SELECT_FLASH:
			nrf_gpio_input_disconnect(CONFIG_IO_FLASH_SCLK);
			nrf_gpio_input_disconnect(CONFIG_IO_FLASH_MOSI);
			nrf_gpio_input_disconnect(CONFIG_IO_FLASH_MISO);
			break;
		
		case SPI_SELECT_ACC:
			nrf_gpio_input_disconnect(CONFIG_IO_ACCEL_SCLK);
			nrf_gpio_input_disconnect(CONFIG_IO_ACCEL_MOSI);
			nrf_gpio_input_disconnect(CONFIG_IO_ACCEL_MISO);
			break;
...
static uint8_t	m_spi_select(spi_select_t sel)
{
	...
	{
		case SPI_SELECT_FLASH:
			// 另一个SPI被禁止后，他的管脚应该怎么配置
		
			nrf_gpio_cfg_input(CONFIG_IO_FLASH_MISO, NRF_GPIO_PIN_PULLUP);
			nrf_gpio_cfg_output(CONFIG_IO_FLASH_SCLK);
			nrf_gpio_cfg_output(CONFIG_IO_FLASH_MOSI);
		
			NRF_SPIX->PSEL.SCK 	= CONFIG_IO_FLASH_SCLK;
			NRF_SPIX->PSEL.MOSI 	= CONFIG_IO_FLASH_MOSI;
			NRF_SPIX->PSEL.MISO 	= CONFIG_IO_FLASH_MISO;
...
//			m_spi_enable();
		...
		case SPI_SELECT_ACC:
			...
			nrf_gpio_cfg_input(CONFIG_IO_ACCEL_MISO, NRF_GPIO_PIN_PULLUP);
			nrf_gpio_cfg_output(CONFIG_IO_ACCEL_MOSI);
			nrf_gpio_cfg_output(CONFIG_IO_ACCEL_SCLK);
		
			NRF_SPIX->PSEL.SCK 	= CONFIG_IO_ACCEL_SCLK;
			NRF_SPIX->PSEL.MOSI 	= CONFIG_IO_ACCEL_MOSI;
			NRF_SPIX->PSEL.MISO 	= CONFIG_IO_ACCEL_MISO;
			NRF_SPIX->FREQUENCY= SPI_FREQUENCY_FREQUENCY_M8;		// 8M bit	MC3630最高只支持4M，且未经验证，所以配置为2M比较妥当
			NRF_SPIX->CONFIG =  (SPI_CONFIG_ORDER_MsbFirst << SPI_CONFIG_ORDER_Pos)|
						(SPI_CONFIG_CPHA_Leading << SPI_CONFIG_CPHA_Pos)|
						(SPI_CONFIG_CPOL_ActiveHigh << SPI_CONFIG_CPOL_Pos);
