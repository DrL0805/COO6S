...
static const uint32_t m_protected_pins = 0
    ...
    | PIN_MASK(CONFIG_IO_IIC_SCL_PIN)
    | PIN_MASK(CONFIG_IO_IIC_SDA_PIN)
//    | PIN_MASK(22)
//    | PIN_MASK(23)
    | PIN_MASK(7)
    | 0;
...
bool drv_board_wakeup_prepare(bool wakeup)
{
    ...
    if ( gpregret== 0xCE) // 从测试模式复位进来
    {
        NRF_GPIO->PIN_CNF[CONFIG_IO_BUTTON_WAKEUP] = ((GPIO_PIN_CNF_SENSE_Disabled    << GPIO_PIN_CNF_SENSE_Pos) \
                             | (GPIO_PIN_CNF_DRIVE_S0S1         << GPIO_PIN_CNF_DRIVE_Pos) \
                             | (GPIO_PIN_CNF_PULL_Disabled      << GPIO_PIN_CNF_PULL_Pos)  \
                             | (GPIO_PIN_CNF_INPUT_Disconnect   << GPIO_PIN_CNF_INPUT_Pos) \
                             | (GPIO_PIN_CNF_DIR_Input          << GPIO_PIN_CNF_DIR_Pos)); 
...
//    DEASSERT_SIGNAL(CONFIG_IO_IIC_SDA_PIN,      1, true);
//    DEASSERT_SIGNAL(CONFIG_IO_IIC_SCL_PIN,      1, true);
